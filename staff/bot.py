...
def handler(message):
	...
	if message.text == 'Включить систему оповещений':
		bot.send_message(message.chat.id, 'Система оповещений включена.\nВы получите уведомление, как только новый аккаунт зарегистрируется.\nЧтобы выключить систему и перейти в главное меню, нажмите на самую нижнюю кнопку.')
		# c = 0
		# for i in sql.execute("SELECT full_name FROM users"):
		# 	c += 1
		while True:
			...
			if e > counter:
				...
				counter = e
			time.sleep(10)
	if message.text == 'Выключить систему оповещений':
			bot.send_message(message.chat.id, 'Система оповещений выключена.\nДобро пожаловать в меню!', reply_markup = markup)
			bot.register_next_step_handler(message, handler)
...
push_off = types.KeyboardButton('Выключить систему оповещений')
markup = types.ReplyKeyboardMarkup(resize_keyboard = True, row_width = 1)
markup.add(watch_db, push_on, push_off)
counter = 0
for i in sql.execute("SELECT full_name FROM users"):
	counter += 1
